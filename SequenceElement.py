...
class LinearCombination(object): 
     ...
     def __str__(self): 
          ...
          """ 
          plus_minus_str = "" if IsZero(self.a) else \
                           " + " if self.b > 0 else " - "
          bstr = "" if IsZero(self.b) else plus_minus_str + str(abs(self.b))
          astr = "%s * " % str(self.a) if not IsOne(self.a) and \
                                          not IsZero(self.a) else ""
          rstr = "%s%s%s" % (astr, "%s" if not IsZero(self.a) else "", bstr)
          return rstr
     ## 
     ...
     def find_linear_combination(index_seq, XY = X, check_all_data = True, 
          ...
          - ``init_xpos`` -- The initial sequence index to use to compute the 
                             form of the linear combination, which is 
                             defined by exactly two elements of the sequence 
          """ 
     ...
     def find_all_linear_combinations(index_seq, seqgen, domain_dim = 1): 
          ...
          for lc in lc_list: 
               ...
               for sidx in range(0, len(index_seq)): 
                    ...
                    if sidx == 0 and IsOne(spseq_value) and len(index_seq[0]) == 0: 
                         continue
